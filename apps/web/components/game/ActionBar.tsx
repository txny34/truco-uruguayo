...
export function ActionBar({ estado, miId }: ActionBarProps) {
  ...
  const puedeCantar = esMiTurno
  const trucoCantado = estado.truco.estado !== 'sin_cantar'
  const hayQueCantarAlgo = trucoCantado && estado.truco.ultimoCantador !== miId

  return (
    <div className="flex flex-wrap gap-2 justify-center">
      {/* Truco */}
      {puedeCantar && !trucoCantado && (
        <button onClick={() => s.emit('CANTAR_TRUCO')}
          className="bg-red-600 hover:bg-red-500 text-white font-bold px-5 py-2 rounded-lg transition-colors">
          Truco
        </button>
      )}
      {puedeCantar && estado.truco.estado === 'cantado' && estado.truco.ultimoCantador !== miId && (
        <button onClick={() => s.emit('CANTAR_RETRUCO')}
          className="bg-orange-600 hover:bg-orange-500 text-white font-bold px-5 py-2 rounded-lg transition-colors">
          Retruco
        </button>
      )}
      {puedeCantar && estado.truco.estado === 'retrucado' && estado.truco.ultimoCantador !== miId && (
        <button onClick={() => s.emit('CANTAR_VALE_CUATRO')}
      ...
      {/* Envido */}
      {puedeCantar && estado.envido.estado === 'sin_cantar' && estado.fase === 'envido' && (
        <>
          <button onClick={() => s.emit('CANTAR_ENVIDO', { tipo: 'envido' })}
            className="bg-blue-600 hover:bg-blue-500 text-white font-bold px-5 py-2 rounded-lg transition-colors">
            Envido
          </button>
          <button onClick={() => s.emit('CANTAR_ENVIDO', { tipo: 'real_envido' })}
            className="bg-blue-700 hover:bg-blue-600 text-white font-bold px-5 py-2 rounded-lg transition-colors">
            Real Envido
          </button>
          <button onClick={() => s.emit('CANTAR_ENVIDO', { tipo: 'falta_envido' })}
      ...
      {estado.flor.jugadoresConFlor.includes(miId) && !estado.flor.resuelta && (
        <button onClick={() => s.emit('CANTAR_FLOR')}
      ...
      {/* Ir al mazo */}
      {esMiTurno && (
        <button onClick={() => s.emit('IR_AL_MAZO')}
